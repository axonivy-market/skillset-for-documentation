...
3. **Output Shape**
    - Group by process file, with heading `#### [process-file-name].p.json`
        - For each callable sub, print a short signature label and a multiline signature code block.
    - **Signature format rules**:
      ...
      - Include **full type names** (not shortened) — e.g., `msgraph.connector.NewMail` not `NewMail`
            | Format label only: `**[name] -> [resultVar]: [resultType]**`
            | Do NOT emit a fenced multiline signature code block. The output MUST include the concise signature label followed by Input/Result lists. This keeps README compact while preserving full parameter details in the lists below.
            - Multiple params separated by `,` and one parameter per line inside the code block
      - If no inputs: state `()` with no params listed
      - If no result: state `-> (none)` or omit result entirely
            - Example label: `**writeMail -> message: com.microsoft.graph.MicrosoftGraphMessage**`
...
- **createMeeting -> meeting: com.microsoft.graph.MicrosoftGraphEvent**
...
- Signature label and parameter/result lists only:
    - Label: `**[name] -> [output var]: [type]**`
    - Do NOT include a fenced multiline signature code block; include detailed Input and Result lists (types, names, descriptions) immediately after the label.
